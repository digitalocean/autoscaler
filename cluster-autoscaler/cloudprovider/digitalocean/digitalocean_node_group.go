...
func toInstanceStatus(nodeState *godo.KubernetesNodeStatus) *cloudprovider.InstanceStatus {
	...
	case "provisioning":
		st.State = cloudprovider.STATE_BEING_CREATED
	case "running":
		st.State = cloudprovider.STATE_RUNNING
	case "draining", "deleting":
		st.State = cloudprovider.STATE_BEING_DELETED
	default:
		st.ErrorInfo = &cloudprovider.InstanceErrorInfo{
			ErrorClass:   cloudprovider.ERROR_OTHER,
